...
class Date_Holidays_Driver_France extends Date_Holidays_Driver
{
    ...
    function _buildHolidays()
    {
        ...
        $this->_addHoliday('france_newYearsDay',
                           $this->_year . '-01-01',
                           'New Year\'s Day');

        $this->_addHoliday('france_dayOfWork',
                           $this->_year . '-05-01',
                           'Day of Work');

        $this->_addHoliday('france_VEDay',
                            $this->_year . '-05-08',
                            'Victory in Europe Day');

        $this->_addHoliday('france_bastilleDay',
                            $this->_year . '-07-14',
                            'Bastille Day');

        $this->_addHoliday('france_armisticeDay',
                            $this->_year . '-11-11',
                            'Armistice Day');

        /* from Christian.php */
        $easterDate = Date_Holidays_Driver_Christian::calcEaster($this->_year);
        $this->_addHoliday('france_easterMonday',
                           $easterDate->add( new DateInterval('P1D')),
                           'Easter Monday');

        $whitsunDate = $this->_addDays($easterDate, 49);
        $ascensionDayDate = $this->_addDays($whitsunDate, -10);
        $this->_addHoliday('france_ascensionDay', $ascensionDayDate, 'Ascension Day');
        $this->_addHoliday('france_whitMonday', $whitsunDate->add( new DateInterval('P1D')), 'Whit Monday');

        $this->_addHoliday('france_mariaAscension',
                           $this->_year . '-08-15',
                           'Ascension of Maria');
        $this->_addHoliday('france_allSaintsDay',
                           $this->_year . '-11-01',
                           'All Saints\' Day');
        $this->_addHoliday('france_christmasDay', $this->_year . '-12-25', 'Christmas Day');
